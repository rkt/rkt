...
This will build the rkt binary and the KVM stage1 aci image in `build-rkt-1.20.0+git/target/bin/`. Depending on the configuration options, it will be `stage1-kvm.aci` (if one hypervisor is set), or `stage1-kvm-lkvm.aci` and `stage1-kvm-qemu.aci` (if you want to have both images built once).
...
```
# rkt run --stage1-name=coreos.com/rkt/stage1-kvm:1.20.0 coreos.com/etcd:v2.0.9
...
```
