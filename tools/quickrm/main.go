...
func getList(file string, proc processor) ([]string, error) {
	fd, err := os.Open(file)
	if err != nil {
		return nil, fmt.Errorf("failed to open %s list %q: %v", proc.kind(), file, err)
	}
	defer fd.Close()
	...
	if !sc.Scan() {
		if err := sc.Err(); err != nil {
			return nil, fmt.Errorf("failed to read %s list %q: %v", proc.kind(), file, err)
		}
		return nil, fmt.Errorf("failed to parse %s list %q: premature end of file", proc.kind(), file)
	}
	num, err := strconv.Atoi(sc.Text())
	if err != nil {
		return nil, fmt.Errorf("failed to parse first line of %s list %q: %v", proc.kind(), file, err)
	}
	proc.setWordsNumber(num)
	for sc.Scan() {
		if err := proc.processWord(sc.Text()); err != nil {
			return nil, fmt.Errorf("failed to process %s from %s list %q: %v", sc.Text(), proc.kind(), file, err)
		}
	}
	if err := sc.Err(); err != nil {
		return nil, fmt.Errorf("failed to read %s list %q: %v", proc.kind(), file, err)
...
func (proc *symlinkProcessor) processWord(symlink string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to evaluate symlinks for symlink dir %q: %v", symlinkDir, err)
	}
...
func (proc *dirProcessor) processWord(dir string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to evaluate symlinks for dir %q: %v", dir, err)
	}
