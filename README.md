...
The escape character ^] is generated by Ctrl-] on a US keyboard, on other keyboard layouts that will differ (for example the Swedish keyboard layout where Ctrl-å on OSX or Ctrl-^ on Windows will generate the correct escape character).
